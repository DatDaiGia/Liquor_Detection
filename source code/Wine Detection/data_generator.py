...
class BatchGenerator:
    ...
    def parse_txt(self,
            ...
            data = pd.read_csv(annotations_path, header=None)
            index = 0
            self.filenames = []
            self.labels = []
           
            while index < len(data):
                file_name = self.images_path + data[0][index]
                number_face = int(data[0][index+1])
                class_id = 1
                coords_index_start = index + 2
                coords_index_end = coords_index_start + number_face
                ...
                labels = []
                for coords in data[0][coords_index_start:coords_index_end]:
                    coords = coords.split(' ')
                    xmin = int(coords[0])
                    ymin = int(coords[1])
                    xmax = int(coords[2]) + xmin
                    ymax = int(coords[3]) + ymin
                    labels.append([class_id, xmin, xmax, ymin, ymax])
                
                index = coords_index_end
    ...
    def load_data_from_xml(self, 
        ...
        for item in xml_list:
            handler = open(item).read()
            soup = BeautifulSoup(handler)
            label = []

            file_name = soup.find('filename').text
            all_names = soup.findAll('name')
            all_xmins = soup.findAll('xmin')
            all_xmaxs = soup.findAll('xmax')
            all_ymins = soup.findAll('ymin')
            all_ymaxs = soup.findAll('ymax')
            n_items = len(all_names)

            for i in range(n_items):
                xmin = all_xmins[i].text.strip()
                xmax = all_xmaxs[i].text.strip()
                ymin = all_ymins[i].text.strip()
                ymax = all_ymaxs[i].text.strip()
                name = all_names[i].text.strip()
                if name == 'Chivas regal':
                    name = 'Chivas Regal'

                if name == 'Jack daniels':
                    name = 'Jack Daniel'

                if name == 'Remy martin':
                    name = 'Remy Martin'
                class_id = int(label_encoder.transform([name]))
                label.append(np.array([class_id, xmin, xmax, ymin, ymax], dtype=np.int))

            self.labels.append(label)
            self.filenames.append("{}/{}/{}".format(self.image_set_path,name, file_name))
    ...
    def generate(self,
        ...
        while True:
            ...
            for filename in self.filenames[current:current+batch_size]:
                # print(len(self.filenames[current:current+batch_size]))
            ...
            # if (len(self.filenames[current:current+batch_size]) != len(self.labels[current:current+batch_size])):
            #     continue
